...
def retrieve_and_expand_query(query: str):
    ...
    # }
    # retrieved_article_ids = [doc.metadata.get("content_id") for doc in retrieved_docs]

    retrieved_article_ids, keywords = retrieve_top_k(query, k=10)
    keywords_map = {f"Document_{i + 1}_Keywords": keyword for i, keyword in enumerate(keywords)}
